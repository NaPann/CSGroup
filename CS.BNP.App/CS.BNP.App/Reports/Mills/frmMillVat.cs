...
namespace CS.BNP.App.Reports.Mills
{
    public partial class frmMillVat : Form
    {
        ...
        public void DGV()
        {
            ...
            var tmpdata = db.vwrpt_TranCreditorMill.Where(w => w.TransactionDate >= _dateStart
     && w.TransactionDate <= _dateEnd && (w.ServiceIsVat.HasValue && w.ServiceIsVat.Value)).AsEnumerable().Select((s, inx) => new
     {
         iNo = inx + 1,
         TransactionDate = s.TransactionDate,
         CreditorServiceID = s.CreditorServiceID,
         CreditorServiceName = s.CreditorServiceName,
         MillTotalAmount = s.ServiceTotalAmount,
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                rptViewer.PrintReport(ReportType.MillVat);
