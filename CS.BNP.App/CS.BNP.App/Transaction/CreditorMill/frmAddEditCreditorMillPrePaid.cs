...
namespace CS.BNP.App.Transaction.Creditor
{
    public partial class frmAddEditCreditorMillPrePaid : Form
    {
        ...
        private void BindingControl()
        {
            ...
            data.Insert(0, new { val = 0, txt = "-- เลือกเจ้าหนี้ ( โรงโม่ ) --" });
        ...
        private void frmAddEditCreditorMillPrePaid_Load(object sender, EventArgs e)
        {
            ...
            if (iType > 0)
            {
                ...
                if (data != null)
                {
                    this.cbCreditorMill.SelectedValue = data.CreditorServiceID;
                ...
                using (var db = new Entity.CSBNPEntities())
                {
                    ...
                    switch (iType)
                    {
                        case 0:
                            ...
                            obj.CreditorServiceID = int.Parse(this.cbCreditorMill.SelectedValue.ToString());
        ...
        private void cbCreditorMill_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (this.cbCreditorMill.SelectedIndex > 0)
            {
                ...
                if (_creditor == null)
                {
                    gloService.Announce("ไม่พบข้อมูลเจ้าหนี้ที่ให้บริการ .. โปรดตรวจสอบ !!", AnnounceType.Warning);
                    this.cbCreditorMill.Select();
                }
                else
                {
                    this.txtCreditorMillName.Text = _creditor.CreditorName + " (" + _creditor.CreditorTel + " : " + _creditor.CreditorAddress + ") ";
                }
                this.Compute();
            }
            else
        ...
        private void Compute()
        {
            if (this.cbCreditorMill.SelectedIndex > 0 && this.cbJob.SelectedIndex > 0)
            {
                ...
                try
                {
                    this.txtBillingAmount.Text = db.tran_Creditor.Where(w => w.CreditorID == creId && w.JobID == jobId).Sum(ss => ss.TotalAmount).ToString("#,0.00");
                }
                catch
                {
