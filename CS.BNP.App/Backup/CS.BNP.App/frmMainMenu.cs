...
namespace CS.BNP.App
{
    public partial class frmMainMenu : Form
    {
        ...
        {            
            ...
            {
                var check = db.tran_CreditorMillPrePaid.Where(w => w.TotalAmount < 0).ToList();
                foreach (var item in check)
                {
                    // Show a sample alert window.
                    AlertInfo info = new AlertInfo("", item.mas_CreditorMill.CreditorMillName + " เงินวาง = <b>" + item.TotalAmount.ToString("#,00") + "</b>", true);
