        ...
        private void btnDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            if (gloService.Sure("คุณต้องการลบข้อมูลรายการนี้ ?") == DialogResult.Yes)
            {
                ...
                using (var db = new Entity.CSBNPEntities())
                {
                    ...
                    {
                        //Mill
                        var delMill = db.tran_CreditorMill.Where(w => w.TranCreditorID == _deldata).FirstOrDefault();
                        if (delMill != null)
                        {
                            //RollBack Amount
                            var roll = db.tran_CreditorMillPrePaid.Where(w => w.CreditorMillID == delMill.CreditorMillID && w.JobID == data.JobID).FirstOrDefault();
                            if (roll != null)
                            {
                                roll.TotalAmount += delMill.TotalAmount;
                            }
                            //
                            db.tran_CreditorMill.Remove(delMill);
                        }
                    }
