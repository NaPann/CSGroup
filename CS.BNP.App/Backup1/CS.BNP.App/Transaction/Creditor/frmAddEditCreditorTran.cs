    ...
    public partial class frmAddEditCreditorTran : Form
    {
                ...
                using (var db = new Entity.CSBNPEntities())
                {
                    ...
                    switch (iType)
                    {
                        case 0:
                            ...
                            {
                                int _mill = modelMill[0].creditormilll_id;
                                var dataMill = db.mas_CreditorMill.Where(w => w.ID == _mill).FirstOrDefault(); 
                                var objMill = new Entity.tran_CreditorMill();
                                objMill.CreditorMillID = dataMill.ID;
                                objMill.CreditorMillCode = dataMill.CreditorMillCode;
                                objMill.CreditorMillName = dataMill.CreditorMillName;
                                objMill.IsVat = modelMill[0].is_vat;
                                objMill.VatAmount = modelMill[0].vat_amount;
                                objMill.VatInt = modelMill[0].vat_int;
                                objMill.NetAmount = modelMill[0].net_amount;
                                objMill.TotalAmount = modelMill[0].total_amount;
                                objMill.TranCreditorID = obj.ID;
                                objMill.Remark = modelMill[0].remark;
                                db.tran_CreditorMill.Add(objMill);

                                //Reduce Mill Advance
                                int _jodId = int.Parse(this.cbJob.SelectedValue.ToString());
                                var reduce = db.tran_CreditorMillPrePaid.Where(w => w.CreditorMillID == dataMill.ID 
                                && w.JobID == _jodId).FirstOrDefault();
                                if (reduce != null)
                                {
                                    reduce.TotalAmount = reduce.TotalAmount - objMill.TotalAmount;
                                }
                                //
                                db.SaveChanges();
                            }
                        ...
                        default:
                            ...
                            if (modelMill.Count() > 0)
                            {
                                int _mill = modelMill[0].creditormilll_id;
                                var dataMill = db.mas_CreditorMill.Where(w => w.ID == _mill).FirstOrDefault();
                                var objMill = db.tran_CreditorMill.Where(w => w.TranCreditorID == iType).FirstOrDefault();
                                objMill.CreditorMillID = dataMill.ID;
                                objMill.CreditorMillCode = dataMill.CreditorMillCode;
                                objMill.CreditorMillName = dataMill.CreditorMillName;
                                objMill.IsVat = modelMill[0].is_vat;
                                ...
                                {
                                    //roll back first
                                    reduce.TotalAmount = reduce.TotalAmount +  modelMill[0].exAmount;
                                    reduce.TotalAmount = reduce.TotalAmount - objMill.TotalAmount;
                                }
                                //
                                db.SaveChanges();
            ...
            if (iType > 0)
            {
                ...
                if (data != null)
                {
                    ...
                    if (dataMill != null)
                    {
                        ...
                        var objMill = new CreditorMill();
                        objMill.creditormilll_id = dataMill.CreditorMillID;
                        objMill.id  = dataMill.ID;
                        objMill.tran_id = dataMill.TranCreditorID;
                        objMill.is_vat = dataMill.IsVat;
                        objMill.net_amount = dataMill.NetAmount;
                        objMill.remark = dataMill.Remark;
                        objMill.total_amount = dataMill.TotalAmount;
                        objMill.vat_int = dataMill.VatInt;
                        objMill.vat_amount = dataMill.VatAmount;
                        objMill.exAmount = dataMill.TotalAmount;
                        modelMill.Add(objMill);

                        if (modelMill != null)
                            this.lblMill.Visible = modelMill.Count() > 0;
        ...
        private void btnAddCreditorMill_Click(object sender, EventArgs e)
        {
            if (this.cbJob.SelectedIndex > 0)
            {
                ...
                frmMill.modelMill = modelMill;
                frmMill.ShowDialog();

                modelMill = frmMill.modelMill;
                //db = new Entity.CSBNPEntities();
                //var data = db.tran_CreditorMill.Where(w=>)
            ...
            {
                gloService.Announce("กรุณาเลือกโครงการก่อนที่จะบันทึกโรงโม่ !!", AnnounceType.Warning);
