...
{
    public partial class frmDebitorViewer : Form
    {
        public frmDebitorViewer()
        {
            InitializeComponent();
        }
        ...
        public void DGV()
        {
            if (this.cbDebitorPeriod.SelectedIndex > 0)
            {
                ...
                var data = db.tran_DebitorRec.Where(w => w.DebitorPeriodID == _condition).ToList();
                foreach (var item in data)
                {
                    this.ds.Tables[0].Rows.Add(this.ds.Tables[0].Rows.Count + 1, item.ID, item.DebitorPeriodID, item.mas_DebitorBillingPeriod.DebitorPeriodCode,
                        ...
                        item.IsReceived, item.CheckedBy, item.CheckedDate, 
                        item.mas_Debtor.DebtorName);
        ...
        private void frmDebitorViewer_Load(object sender, EventArgs e)
        {
            BuildDS();
            ClearForm();
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                rptViewer.keyId = int.Parse(this.gridView.GetRowCellValue(this.gridView.FocusedRowHandle, gridView.Columns["DebitorPeriodID"]).ToString());
                rptViewer.PrintReport(ReportType.SummaryDebitor);
